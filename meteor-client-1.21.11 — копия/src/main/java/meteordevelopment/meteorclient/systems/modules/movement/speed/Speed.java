...
import meteordevelopment.meteorclient.systems.modules.movement.speed.modes.ElytraSpoof;
...
public class Speed extends Module {
    ...
    public final Setting<Boolean> autoJump = sgGeneral.add(new BoolSetting.Builder()
        .name("auto-jump")
        .description("Automatically jumps to gain speed while ElytraSpoof is active.")
        .visible(() -> speedMode.get() == SpeedModes.ElytraSpoof)
        .defaultValue(true)
        .build()
    );

    public final Setting<Double> spoofSpeed = sgGeneral.add(new DoubleSetting.Builder()
        .name("spoof-speed")
        .description("Horizontal speed applied while in the air with ElytraSpoof.")
        .visible(() -> speedMode.get() == SpeedModes.ElytraSpoof)
        .defaultValue(0.9)
        .min(0)
        .sliderMax(5)
    ...
    private void onSpeedModeChanged(SpeedModes mode) {
        switch (mode) {
            ...
            case ElytraSpoof -> currentMode = new ElytraSpoof();
        }
    }

    private boolean stopSpeed() {
        // ElytraSpoof intentionally spoofs gliding, so it must keep running even when
        // the server mirrors the fall-flying flag back to the client.
        if ((speedMode.get() != SpeedModes.ElytraSpoof && mc.player.isGliding()) || mc.player.isClimbing() || mc.player.getVehicle() != null) return true;
